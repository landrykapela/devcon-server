...
class User {
    ...
    isDeveloper() {
        return (this.user_type === TYPE_DEV);
    }
    isClient() {
    ...
    login(password) {
        return new Promise((resolve, reject) => {
            let con = db.connect();
            db.findSingleRecord(con, "user", [{ variable: "email", operation: "=", value: this.email }])
                .then((result) => {
    ...
    find() {
        return new Promise((resolve, reject) => {
            let con = db.connect();
            db.findSingleRecord(con, "user", [{ variable: "id", operation: "=", value: this.id }])
                .then((result) => {
    ...
    register() {
        return new Promise((resolve, reject) => {
            let con = db.connect();
            let tableName = "user";
            let data = { email: this.email, name: this.name, user_type: this.user_type, password: this.password };
            console.log("user register(): " + JSON.stringify(data));
            let fields = [{ name: "email", type: "varchar", field_size: 255, required: true }, { name: "name", type: "varchar", field_size: 255, required: true }, { name: "user_type", type: "int", field_size: 1, required: true }, { name: "password", type: "varchar", field_size: 255, required: true }];
